...
코드 수정 후 반드시 아래 순서를 자동으로 실행:

1. 코드 리뷰 — 수정한 파일에 import 에러, 오타, 누락 없는지 확인
2. 빌드 테스트 — `cd C:\Kane-github\orbit\apps\web && npx next build`
3. 빌드 성공 시 자동 배포:
   ```bash
   ...
   ```
4. 빌드 실패 시 — 에러 분석 후 자동 수정, 다시 빌드. 성공할 때까지 반복.
5. 결과 보고
...
- postinstall에서 BlockSuite 아이콘 오타 자동 패치 유지
