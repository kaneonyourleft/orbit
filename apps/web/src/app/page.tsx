...
export default function Home() {
  ...
  const handleRealtimeUpdate = useCallback((payload: { eventType: string; new: any; old: any }) => {
    if (payload.eventType === 'INSERT') setRows(prev => prev.find(r => r.id === payload.new.id) ? prev : [...prev, payload.new]);
    else if (payload.eventType === 'UPDATE') setRows(prev => prev.map(row => row.id === payload.new.id ? payload.new : row));
    else if (payload.eventType === 'DELETE') setRows(prev => prev.filter(row => row.id !== payload.old.id));
  }, [setRows]);
        ...
        <main className="flex-1 bg-white flex flex-col overflow-auto min-w-0">
          ...
          <div className="flex flex-col min-h-full">
            <div className="px-6 pt-6 pb-3 shrink-0 bg-white sticky top-0 z-20 border-b border-zinc-100">
              ...
              <ViewSwitcher activeView={activeView as any} onViewChange={setActiveView} extraViews={extraViews} />
